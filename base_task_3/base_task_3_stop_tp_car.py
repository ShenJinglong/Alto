...
class base_task_3_stop_tp_car_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        base_task_3_stop_tp_car(flight)

    def exit_state(self, flight):
        pass

base_task_3_stop_tp_car_counter = 0

def base_task_3_stop_tp_car(flight):
    # start_time = cv2.getTickCount()
    global base_task_3_stop_tp_car_counter
    ...
    try:
        ...
        if len(circles[:, 0, :]) == 1:
            base_task_3_stop_tp_car_counter += 1
    ...
    if base_task_3_stop_tp_car_counter == params.BASE_TASK_3_STOP_TP_CAR_NUM:
        base_task_3_stop_tp_car_counter = 0
        flight.next_state = macro.BASE_TASK_3_LAND
        print('>>> [!] BASE_TASK_3_STOP_TP_CAR -> BASE_TASK_3_LAND')
