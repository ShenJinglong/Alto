...
class base_task_3_take_off_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        base_task_3_take_off(flight)

    def exit_state(self, flight):
        pass

base_task_3_take_off_counter = 0
lose_target_counter = 0

def base_task_3_take_off(flight):
    # start_time = cv2.getTickCount()
    global base_task_3_take_off_counter
    ...
    try:
        ...
        if len(circles[:, 0, :]) == 1:
            base_task_3_take_off_counter += 1
            lose_target_counter = 0
    ...
    if base_task_3_take_off_counter == params.BASE_TASK_3_TAKE_OFF_NUM:
        base_task_3_take_off_counter = 0
        flight.next_state = macro.BASE_TASK_3_LEAVE_START_POINT
        print('>>> [!] BASE_TASK_3_TAKE_OFF -> BASE_TASK_3_LEAVE_START_POINT')
