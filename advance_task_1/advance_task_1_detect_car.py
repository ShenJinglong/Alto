...
class advance_task_1_detect_car_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        advance_task_1_detect_car(flight)

    def exit_state(self, flight):
        pass

advance_task_1_detect_car_counter = 0

def advance_task_1_detect_car(flight):
    # start_time = cv2.getTickCount()
    global advance_task_1_detect_car_counter
    ...
    try:
        ...
        if len(circles[:, 0, :]) == 1 and circles[0, 0, 0] > global_params.IMAGE_WIDTH / 2:
            advance_task_1_detect_car_counter += 1
        else:
            advance_task_1_detect_car_counter = 0
    ...
    if advance_task_1_detect_car_counter == params.ADVANCE_TASK_1_DETECT_CAR_NUM:
        advance_task_1_detect_car_counter = 0
        flight.next_state = macro.ADVANCE_TASK_1_BRAKE
        print('>>> [!] ADVANCE_TASK_1_DETECT_CAR -> ADVANCE_TASK_1_BRAKE')
