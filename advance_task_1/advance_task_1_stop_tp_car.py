...
class advance_task_1_stop_tp_car_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        advance_task_1_stop_tp_car(flight)

    def exit_state(self, flight):
        pass

advance_task_1_stop_tp_car_counter = 0

def advance_task_1_stop_tp_car(flight):
    # start_time = cv2.getTickCount()
    global advance_task_1_stop_tp_car_counter
    ...
    try:
        ...
        if len(circles[:, 0, :]) == 1:
            # advance_task_1_stop_tp_car_counter += 1
    ...
    if advance_task_1_stop_tp_car_counter == params.ADVANCE_TASK_1_STOP_TP_CAR_NUM:
        advance_task_1_stop_tp_car_counter = 0
        flight.next_state = macro.ADVANCE_TASK_1_LAND
        print('>>> [!] ADVANCE_TASK_1_STOP_TP_CAR -> ADVANCE_TASK_1_LAND')
