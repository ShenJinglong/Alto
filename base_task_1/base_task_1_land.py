...
class base_task_1_land_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        base_task_1_land(flight)

    def exit_state(self, flight):
        pass

base_task_1_land_counter = 0

def base_task_1_land(flight):
    global base_task_1_land_counter
    ...
    base_task_1_land_counter += 0
    if base_task_1_land_counter == params.BASE_TASK_1_LAND_NUM:
        base_task_1_land_counter = 0
        flight.next_state = macro.MODE_SELECTOR
        print('>>> [!] BASE_TASK_1_LAND -> MODE_SELECTOR')
